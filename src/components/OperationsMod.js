const OperationsMod = ({ setPage, teachers, students, shifts }) => {
  const act = (teachers || []).filter(
  ...
  const ctry = new Set((students || []).map((s) => s.country).filter(Boolean))
    .size;
  const shList = shifts || [];
  const pendSh = shList.length;
  const subjCount =
    typeof ALL_SUBJECTS !== "undefined" ? ALL_SUBJECTS.length : 7;
  const cards = [
    {
      id: "timetable",
      label: "Timetable",
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        "Coming as tabs inside child modules",
      ),
      React.createElement(
        "div",
        {
        ...
        [
          "\u2713 Zoom Links \u2192 done (in Teachers)",
          "\u2713 Student Monthly Attendance Grid \u2192 done (in Students)",
        ].map((item) =>
          React.createElement(
            "span",
            {
