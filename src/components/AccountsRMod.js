const AccountsRMod = ({ students, arPayments, setArPayments }) => {
  ...
  const monthPays = pays.filter(
    (p) => String(p.paidDate || "").slice(0, 7) === month,
  );
  ...
  const paidByStudent = (sid) =>
    monthPays
      .filter((p) => String(p.studentId) === String(sid))
  ...
  const savePay = () => {
    ...
    if (modal.type === "edit")
      setArPayments(pays.map((p) => (p.id === form.id ? rec : p)));
    else
      setArPayments([
        ...pays,
        {
  ...
  const collRate =
    expectedTotal > 0
      ? Math.round(
          (Math.min(totalCollected, expectedTotal) / expectedTotal) * 100,
        )
      : 0;
  const gatewayTally = (() => {
    const t = {};
  ...
  const tabs = [
    ["paying", "Monthly Paying Students"],
    ["feerec", "Monthly Fee Record"],
    ["recovery", "Monthly Recovery Record"],
    ["daily", "Daily Recovery Sheet"],
    ["posting", "Payment Posting Sheet"],
    ["defaulter", "Monthly Defaulter List"],
  ];
  const exportCSV = () => {
    let cols, rows, name;
    if (tab === "paying") {
      ...
      rows = monthPays
        .filter((p) =>
          matchS([p.studentName, p.parent, p.course, p.gateway, p.receipt]),
        )
        .map((p) => [
          p.studentName,
          p.parent,
      ...
      rows = feeRows
        .filter((r) => matchS([r.s.name, r.s.parent, r.s.course]))
        .map((r) => [
          r.s.name,
          r.s.parent,
          r.s.course,
      ...
      rows = recoveryPays
        .filter((p) => matchS([p.studentName, p.parent, p.gateway]))
        .map((p) => [
          p.studentName,
          p.parent,
          num(p.amount),
  ...
  if (tab === "paying") {
    ...
    content = React.createElement(
      ...
      Table(
        ...
        "No payments recorded for " +
          monthLabel +
          ". Post payments from the Payment Posting Sheet tab.",
      ),
    );
  } else if (tab === "feerec") {
    const rows = feeRows.filter((r) =>
      matchS([r.s.name, r.s.parent, r.s.course]),
    );
    content = React.createElement(
      "div",
      null,
      React.createElement(
        ...
        React.createElement(SC, {
          ...
          value: collRate + "%",
          color:
            collRate >= 80 ? c.success : collRate >= 50 ? c.warn : c.danger,
        }),
      ),
      Table(
        rows,
    ...
    const rows = recoveryPays.filter((p) =>
      matchS([p.studentName, p.parent, p.gateway]),
    );
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "span",
          null,
          "Recovery payments are collections against past dues. When posting a payment, tick ",
          React.createElement(
            "strong",
            {
              style: {
              ...
              React.createElement(
                ...
                React.createElement("div", {
                  style: {
                    width:
                      (totalCollected
                        ? Math.round((d.sum / totalCollected) * 100)
                        : 0) + "%",
                    height: "100%",
    ...
    const rows = defaulters.filter((r) =>
      matchS([r.s.name, r.s.parent, r.s.course]),
    );
    ...
    content = React.createElement(
      ...
      Table(
        ...
        "No defaulters \u2014 everyone with a set fee has paid for " +
          monthLabel +
          ". \u{1F389}",
      ),
    );
    ...
    const rows = pays
      ...
      .sort((a, b) => (String(a.paidDate) < String(b.paidDate) ? 1 : -1));
    content = React.createElement(
      "div",
      null,
      React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "record every fee payment here. Posted payments flow into Monthly Paying Students, Fee Record, Recovery, Daily Sheet and clear students off the Defaulter List automatically.",
        ),
      ),
      Table(
        rows,
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Fee collection, reconciliation, recovery & defaulters",
        ),
      ),
      React.createElement(
        "div",
    ...
    content,
    modal &&
      (modal.type === "post" || modal.type === "edit") &&
      React.createElement(
        "div",
        {
          style: {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                modal.type === "edit" ? "Edit Payment" : "Post Payment",
              ),
            ),
            React.createElement(
              "button",
            ...
            form.studentId &&
              ...
              React.createElement(
                ...
                "Course: " +
                  ...
                  (expected(
                    roster.find(
                      (x) => String(x.id) === String(form.studentId),
                    ) || {},
                  )
                    ? " \xB7 Expected fee: " +
                      fmt(
          ...
          React.createElement(
            ...
            "Recovery payment (collection against a past due)",
          ),
          React.createElement(Inp, {
            label: "Notes",
            value: form.notes || "",
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "edit" ? "Save Changes" : "Post Payment",
            ),
          ),
        ),
      ),
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "Remove the ",
            fmt(modal.data.amount),
            " ",
            modal.data.currency || "USD",
            " payment from ",
            React.createElement(
              "strong",
              {
