const LoginScreen = ({
  ...
  const [customRoleName, setCustomRoleName] = useState(customRolesList[0] ? customRolesList[0].name : "");
  ...
  const isWFH = pendingUser?.role === "teacher" && pendingUser?.location === "WFH";
  const buildUser = role => {
    ...
    if (role === "parent") {
      const pid = String(parentIdInput || "").trim().toUpperCase();
      if (!pid) return null;
      const stu = (students || []).find(s => String(s.parentId || "").toUpperCase() === pid);
      if (!stu) return null;
      return {
        role: "parent",
  ...
  const tryLogin = role => {
    ...
    if (role === "superadmin") {
      ...
      if (window.addAuditEntry) window.addAuditEntry({
        user: "Mohsin",
        role: "superadmin",
        time: new Date().toISOString(),
        network: "Direct",
  ...
  const onComplete = () => {
    ...
    LOGIN_AUDIT.unshift({
      id: Date.now(),
      user: pendingUser.role === "teacher" ? pendingUser.name + " (" + pendingUser.teacherCode + ")" : pendingUser.name,
  ...
  if (step === "network") return React.createElement(NetworkScanScreen, {
    onConnected: onNetworkConnected,
    onCancel: restart,
    isWFH: isWFH,
    userRole: pendingUser?.role
  });
  if (step === "biometric") return React.createElement(BiometricScreen, {
    user: pendingUser,
    network: network,
    onVerified: onBioVerified,
    onCancel: restart
  });
  if (step === "attendance") return React.createElement(AttendanceMarkedScreen, {
    user: pendingUser,
    network: network,
    method: bioMethod,
    photo: photo,
  ...
  return React.createElement("div", {
  ...
  }, React.createElement("style", null, "@keyframes spin{to{transform:rotate(360deg)}}"), React.createElement("div", {
    style: {
      width: "100%",
      maxWidth: 480
    }
  ...
  }, "Let's Learn Quran Academy"), React.createElement("p", {
    style: {
      color: c.textSec,
      fontSize: 12,
      margin: 0
    }
  }, "Secure CRM Portal \xB7 Multi-factor authentication")), step === "role" && React.createElement("div", {
    style: {
      background: c.bgCard,
  ...
  }, "Protected by Network Gate + Biometric Verification + Audit Log")), React.createElement("h3", {
    style: {
      color: c.text,
      fontSize: 14,
      margin: "0 0 14px",
    ...
    const permList = Object.entries(cr.permissions || {}).filter(([k, v]) => v).map(([k, v]) => k);
    const permSummary = permList.length === 0 ? "No permissions granted" : permList.length <= 2 ? permList.join(", ") : permList.length + " permissions";
    return {
      id: "custom:" + cr.name,
  ...
  })].map(r => React.createElement("button", {
    key: r.id,
    onClick: () => {
      if (r._isCustom) {
  ...
  }, r.desc)), React.createElement(ChevronRight, {
    size: 16,
    color: c.textMuted
  })))), step === "superadmin" && React.createElement("div", {
    style: {
      background: c.bgCard,
  ...
  }, "Super Admin Login")), React.createElement("div", {
    style: {
  ...
  }, 'Demo: leave password blank or use "admin123"'), React.createElement(Inp, {
    label: "Password",
    type: "password",
    value: pwd,
    onChange: setPwd,
    ...
    icon: Shield
  }, "Sign In as Super Admin")), step === "teamlead" && React.createElement("div", {
    style: {
      background: c.bgCard,
  ...
  }, "Team Lead Portal")), React.createElement(Inp, {
  ...
  }, "Next: Network check \u2192 Biometric \u2192 Auto-attendance"), React.createElement(Btn, {
    onClick: () => tryLogin("teamlead"),
    icon: Users
  }, "Continue \xB7 Verify Identity")), step === "teacher" && (() => {
    const t = teachers.find(x => x.id === Number(teacherId));
    return React.createElement("div", {
      style: {
        background: c.bgCard,
    ...
    }, "Teacher Portal")), React.createElement("div", {
      style: {
        marginBottom: 10
      }
    }, React.createElement("div", {
    ...
    }, "Select Your Profile"), React.createElement("select", {
      value: teacherId,
    ...
    }, teachers.map(t => React.createElement("option", {
      key: t.id,
      value: t.id
    }, t.name, " (", t.code, ") \u2014 ", t.location)))), t && React.createElement("div", {
      style: {
        background: t.location === "WFH" ? c.warnBg : c.successBg,
        border: "1px solid " + (t.location === "WFH" ? c.warn : c.success) + "44",
        borderRadius: 8,
        padding: "10px 12px",
        marginBottom: 12
      }
      ...
      text: t.location === "WFH" ? "🏠 Work From Home" : "🏢 On-site (IBA)",
    ...
    }, t.location === "WFH" ? "Security: Academy WiFi OR Home WiFi/Mobile Data with OTP verification + GPS lock + photo capture" : "Security: Strict — Must be on academy WiFi (LLQA 5G/2G/IBA) + biometric + photo capture")), React.createElement("div", {
      style: {
        background: c.successBg,
        borderRadius: 8,
        padding: "8px 12px",
    ...
    }, "4 steps: Network \u2192 Biometric (silent photo) \u2192 Audit \u2192 Auto-attendance"), error && React.createElement("div", {
      style: {
        color: c.danger,
        fontSize: 11,
        marginBottom: 10
  ...
  })(), step === "parent" && React.createElement("div", {
    style: {
      background: c.bgCard,
  ...
  }, "Parent Portal Login")), React.createElement("div", {
    style: {
      background: c.bgDeep,
      border: "1px solid " + c.border,
      borderRadius: 8,
  ...
  }, "Enter the unique Parent ID provided by the academy. You will see your child\u0027s progress, attendance, invoices, and can send notes to teachers."), React.createElement("label", {
    style: {
      display: "block",
      color: c.textSec,
      fontSize: 10,
  ...
  }, "Parent ID"), React.createElement("input", {
    value: parentIdInput,
    ...
    onKeyDown: e => {
      if (e.key === "Enter") {
        ...
        if (u) {
          ...
          setError("Parent ID not found. Please verify with the academy.");
        }
      }
    }
  }), React.createElement("button", {
  ...
  }, "Enter Parent Portal"), error ? React.createElement("div", {
    style: {
      color: c.danger,
      fontSize: 11,
      marginTop: 10,
  ...
  }, error) : null), step === "custom-prompt" && (() => {
    const roleRec = customRolesList.find(r => r.name === customRoleName);
    if (!roleRec) {
      return React.createElement("div", {
        style: {
          background: c.bgCard,
      ...
      }, "Role not found."), React.createElement("button", {
    ...
    const permList = Object.entries(roleRec.permissions || {}).filter(([k, v]) => v).map(([k, v]) => k);
    return React.createElement("div", {
      style: {
        background: c.bgCard,
    ...
    }, customRoleName + " (Custom Role)")), roleRec.desc ? React.createElement("div", {
      style: {
        color: c.textSec,
        fontSize: 11,
        marginBottom: 12,
    ...
    }, roleRec.desc) : null, React.createElement("div", {
      style: {
        color: c.textSec,
        fontSize: 10,
        marginBottom: 6,
    ...
    }, "This role has access to:"), permList.length > 0 ? React.createElement("div", {
      style: {
        display: "flex",
        flexWrap: "wrap",
        gap: 4,
    ...
    }, "\u26A0 No permissions granted to this role. Contact Super Admin."), React.createElement("label", {
      style: {
        display: "block",
        color: c.textSec,
        fontSize: 10,
    ...
    }, "Enter Portal"), error ? React.createElement("div", {
      style: {
        color: c.danger,
        fontSize: 11,
        marginTop: 10,
