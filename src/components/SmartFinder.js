const SmartFinder = () => {
  ...
  const allSlots = React.useMemo(() => {
    ...
    ["Morning", "Evening", "Night", "Weekend"].forEach((sh) =>
      TT_DATA[sh].slots.forEach((t) => s.add(t)),
    );
  ...
  const results = React.useMemo(() => {
    ...
    ["Morning", "Evening", "Night", "Weekend"].forEach((sh) => {
      const sd = TT_DATA[sh];
      const ds =
        sh === "Weekend" ? ["Sat", "Sun"] : ["Mon", "Tue", "Wed", "Thu", "Fri"];
      sd.teachers.forEach((t) => {
        if (fTeach && !t.name.toLowerCase().includes(fTeach.toLowerCase()))
          return;
          ...
          sd.slots.forEach((sl) => {
            ...
            if (fm === "freeslots") {
              ...
              if (
                ...
                cell.s &&
                (fSearch === "" ||
                  cell.s.toLowerCase().includes(fSearch.toLowerCase()))
              )
                rows.push({
                  shift: sh,
                  teacher: t.name,
              ...
              if (
                ...
                cell.c &&
                (fSearch === "" ||
                  cell.c.toLowerCase().includes(fSearch.toLowerCase()))
              )
                rows.push({
                  shift: sh,
                  teacher: t.name,
              ...
              if (
                ...
                cell.l &&
                (fSearch === "" ||
                  cell.l.toLowerCase().includes(fSearch.toLowerCase()))
              )
                rows.push({
                  shift: sh,
                  teacher: t.name,
  ...
  const uS =
    fm !== "freeslots"
      ? new Set(results.map((r) => r.student).filter(Boolean)).size
      : 0;
  const showSrch =
    fm === "findstudent" || fm === "findcourse" || fm === "findparent";
  const modes = [
    {
      k: "freeslots",
      l: "Free Slots",
  ...
  const hdr =
    fm === "freeslots"
      ? ["Shift", "Teacher", "Code", "Location", "Lead", "Day", "Time (PKT)"]
      : [
          "Shift",
          "Teacher",
          "Code",
    ...
    React.createElement(
          ...
          React.createElement(
            "button",
            {
              ...
              style: {
                ...
                borderRadius: 8,
                border:
                  fm === m.k ? "1px solid " + m.col : "1px solid transparent",
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          fm === "freeslots"
            ? "\u2713 " + results.length + " free slots found"
        ...
        React.createElement(
          ...
          uT +
            ...
            (uT !== 1 ? "s" : "") +
            " " +
            (fm === "freeslots" ? "available" : "matched"),
        ),
        fm !== "freeslots" && uS > 0
          ? React.createElement(
              "span",
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Showing " + Math.min(results.length, 500) + " of " + results.length,
        ),
      ),
      React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            results.slice(0, 500).map((r, i) => {
              const shCol = (r.shift || "").includes("Morning")
                ? c.warn
                : (r.shift || "").includes("Evening")
                  ? c.purple
                  : (r.shift || "").includes("Night")
                    ? c.accent
                    : c.cyan;
              return React.createElement(
                "tr",
