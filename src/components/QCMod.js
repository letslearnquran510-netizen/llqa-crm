const QCMod = ({ teachers, qcViolations, setQcViolations }) => {
  ...
  const save = () => {
    ...
    if (modal.type === "edit")
      setQcViolations(viols.map((v) => (v.id === form.id ? rec : v)));
    else
      setQcViolations([
        ...viols,
        {
  ...
  const exportCSV = () => {
    ...
    if (tab === "byteacher") {
      ...
      rows = viols
        .filter((v) => matchS([v.teacher, v.vtype, v.description, v.officer]))
        .map((v) => [
          v.date,
          v.teacher,
          v.vtype,
  ...
  if (tab === "log") {
    ...
    content = React.createElement(
      ...
      Table(
        rows,
        logCols(true),
        "No violations logged. Click \u201CLog Violation\u201D to record a QC issue.",
      ),
    );
  } else if (tab === "open") {
    const rows = openCases
      .filter((v) => matchS([v.teacher, v.vtype, v.description]))
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "span",
          null,
          "Unresolved violations sorted by severity. Open the editor to mark a case ",
          React.createElement(
            "strong",
            {
              style: {
      ...
      Table(
        rows,
        logCols(true),
        "No open cases \u2014 all violations are resolved. \u{1F389}",
      ),
    );
              ...
              React.createElement(
                ...
                React.createElement("div", {
                  style: {
                    width:
                      (finesIssued
                        ? Math.round((x.fine / finesIssued) * 100)
                        : 0) + "%",
                    height: "100%",
                    background: c.danger,
                  },
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Teacher violations, fines, case tracking & analytics",
        ),
      ),
      React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "edit" ? "Save Changes" : "Log Violation",
            ),
          ),
        ),
      ),
