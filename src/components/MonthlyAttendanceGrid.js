const MonthlyAttendanceGrid = ({ students, setStudents }) => {
  ...
  const todayDay =
    todayCheck.getFullYear() === year && todayCheck.getMonth() === month
      ? todayCheck.getDate()
      : -1;
  ...
  const teacherOpts = [
    "all",
    ...Array.from(
      new Set(activeStudents.map((s) => s.teacher).filter(Boolean)),
    ).sort(),
  ];
  ...
  const rowSummary = (student) => {
    ...
    Object.keys(log).forEach((k) => {
      if (k.indexOf(prefix) === 0) {
        if (log[k] === "present") p++;
        else if (log[k] === "absent") a++;
        else if (log[k] === "late") l++;
        else if (log[k] === "leave") v++;
      }
    });
    const total = p + a + l + v;
  ...
  const exportCSV = () => {
    ...
    filtered.forEach((s) => {
      const cells = days.map((d) =>
        statusLetter((s.attendanceLog || {})[monthKey(d)]),
      );
  ...
  const todayPresent = activeStudents.filter(
    (s) => (s.attendanceLog || {})[monthKey(todayDay)] === "present",
  ).length;
  const todayAbsent = activeStudents.filter(
    (s) => (s.attendanceLog || {})[monthKey(todayDay)] === "absent",
  ).length;
  const todayLate = activeStudents.filter(
    (s) => (s.attendanceLog || {})[monthKey(todayDay)] === "late",
  ).length;
  const navBtnStyle = {
    padding: "6px 12px",
    background: c.bgDeep,
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      [
        ["Students Visible", filtered.length, c.accent, Users],
        [
          "Today \u00B7 Present",
          todayDay > 0 ? todayPresent : "\u2014",
          c.success,
          Check,
        ],
        [
          "Today \u00B7 Absent",
          todayDay > 0 ? todayAbsent : "\u2014",
          c.danger,
          UserX,
        ],
        [
          "Today \u00B7 Late",
          todayDay > 0 ? todayLate : "\u2014",
          c.warn,
          Clock,
        ],
      ].map((a) =>
    ...
    React.createElement(
      ...
      [
        ["present", "Present", c.success],
        ["absent", "Absent", c.danger],
        ["late", "Late", c.warn],
        ["leave", "Leave", c.accent],
      ].map((a) =>
        React.createElement(
          "span",
          {
      ...
      React.createElement(
        ...
        "Click cell to cycle \u2192 Click day# in header to mark all visible students present",
      ),
    ),
    React.createElement(
      "div",
                  ...
                  days.map((d) => {
                    ...
                    return React.createElement(
                      "th",
                      {
                        ...
                        title:
                          "Click to mark all visible students PRESENT on day " +
                          d,
                        style: {
                          position: "sticky",
                          top: 0,
                        ...
                        "td",
                        {
                          ...
                          style: {
                            ...
                            borderLeft: weekend
                              ...
                              : "none",
                            background:
                              weekend && !col ? c.bgDeep + "44" : "transparent",
                            cursor: "pointer",
                            position: "relative",
                          },
                        },
