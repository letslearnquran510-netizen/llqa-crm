const TrainingMod = ({ teachers, trainingPrograms, setTrainingPrograms }) => {
  ...
  const hoursDelivered = completedProgs.reduce(
    (a, p) => a + num(p.duration),
    0,
  );
  const totalEnroll = allAtt.length;
  const complRate =
    totalEnroll > 0 ? Math.round((certified.length / totalEnroll) * 100) : 0;
  const oAdd = () => {
    setForm({
      topic: "",
      category: "Onboarding",
  ...
  const save = () => {
    ...
    if (modal.type === "edit")
      setTrainingPrograms(progs.map((p) => (p.id === form.id ? rec : p)));
    else
      setTrainingPrograms([
        ...progs,
        {
  ...
  const exportCSV = () => {
    ...
    if (tab === "certified") {
      ...
      rows = certified
        .filter((a2) => matchS([a2.name, a2.program, a2.trainer]))
        .map((a2) => [
          a2.name,
          a2.program,
          a2.category,
      ...
      rows = progs
        .filter((p) => matchS([p.topic, p.trainer, p.category]))
        .map((p) => [
          p.topic,
          p.category,
          p.trainer,
          ...
          num(p.duration),
          (p.attendees || []).length,
          (p.attendees || []).filter((a2) => a2.status === "Completed").length,
          p.status,
          p.materials,
          p.notes,
        ]);
  ...
  if (tab === "programs") {
    ...
    const rows = certified
      .filter((a) => matchS([a.name, a.program, a.trainer]))
      .sort((a, b) => (String(a.date) < String(b.date) ? 1 : -1));
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(SC, {
          icon: Calendar,
          label: "This Month",
          value: certified.filter(
            (a) => String(a.date || "").slice(0, 7) === thisMonth,
          ).length,
          color: c.cyan,
        }),
        React.createElement(SC, {
      ...
      React.createElement(
        ...
        React.createElement(
          "span",
          null,
          "Auto-generated from every attendee marked ",
          React.createElement(
            "strong",
            {
              style: {
          ...
          " in a program \u2014 mark completion inside a program and it appears here instantly.",
        ),
      ),
      Table(
        rows,
    ...
    const upcoming = progs
      ...
      .filter((p) => matchS([p.topic, p.trainer]))
      .sort((a, b) => (String(a.date) < String(b.date) ? -1 : 1));
    const week = upcoming.filter((p) => {
      if (!p.date) return false;
      const d = Math.round(
        (new Date(p.date + "T00:00:00") - new Date(today + "T00:00:00")) /
          864e5,
      );
      return d >= 0 && d <= 7;
    });
      ...
      Table(
        ...
        [
          ...
          },
          {
            h: "Duration",
            render: (p) =>
              num(p.duration) ? num(p.duration) + " h" : "\u2014",
          },
          {
            h: "Enrolled",
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "span",
          null,
          "Per-person training coverage \u2014 who has completed what. Use it to spot ",
          React.createElement(
            "strong",
            {
              style: {
            ...
            untrained + " teacher(s) with no training yet",
          ),
          ".",
        ),
      ),
              ...
              React.createElement(
                ...
                React.createElement("div", {
                  style: {
                    width:
                      (x.enrolled
                        ? Math.round((x.completed / x.enrolled) * 100)
                        : 0) + "%",
                    height: "100%",
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(SC, {
          ...
          value: complRate + "%",
          color:
            complRate >= 70 ? c.success : complRate >= 40 ? c.warn : c.danger,
        }),
        React.createElement(SC, {
          icon: Clock,
                  ...
                  React.createElement(
                    ...
                    React.createElement("div", {
                      style: {
                        width:
                          (progs.length
                            ? Math.round((x.n / progs.length) * 100)
                            : 0) + "%",
                        height: "100%",
                        background: c.accent,
                      },
      ...
      React.createElement(
        ...
        "Trainer Leaderboard (completed programs)",
      ),
      Table(
        trainers,
        [
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Programs, certifications, skills matrix & analytics \u2014 fully automated",
        ),
      ),
      React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              "Attendees & Completion (" + (form.attendees || []).length + ")",
            ),
            React.createElement(
              "div",
              {
            ...
            (form.attendees || []).length === 0
              ? React.createElement(
                  ...
                  "No attendees yet. Add teachers or candidates, then set each one's completion status and score.",
                )
              : React.createElement(
                  "div",
                  {
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "edit" ? "Save Program" : "Create Program",
            ),
          ),
        ),
      ),
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            " and its ",
            (modal.data.attendees || []).length,
            " attendee record(s)? This cannot be undone.",
          ),
          React.createElement(
            "div",
            {
