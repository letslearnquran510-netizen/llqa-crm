const CustomMultiSelect = ({ options, value, onChange, label }) => {
  ...
  const toggleVal = (v) => {
    if (selectedVals.includes(v)) onChange(selectedVals.filter((x) => x !== v));
    else onChange([...selectedVals, v]);
  };
    ...
    React.createElement(
      "div",
      {
        ...
        style: {
          ...
          borderRadius: 6,
          color: selectedVals.length > 0 ? c.text : c.textSec,
          fontSize: 12,
          cursor: "pointer",
          display: "flex",
      ...
      selectedVals.length > 0 ? selectedVals.join(", ") : "Select subjects...",
      React.createElement(ChevronDown, { size: 14, color: c.textSec }),
    ),
    open &&
      React.createElement(
        "div",
          ...
          React.createElement(
            "div",
            {
              ...
              style: {
                ...
                cursor: "pointer",
                fontSize: 12,
                color: selectedVals.includes(opt) ? c.accent : c.textSec,
                background: selectedVals.includes(opt)
                  ? c.bgHover
                  : "transparent",
                display: "flex",
                alignItems: "center",
            ...
            selectedVals.includes(opt) &&
              React.createElement(Check, { size: 14, color: c.accent }),
          ),
        ),
      ),
  );
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                ["Morning", "Evening", "Night", "Weekend"].map((s) => {
                  const currentShifts = (form.shift || "Night")
                    .split(",")
                    .map((x) => x.trim());
