const ProcurementMod = ({
  ...
  const stockState = (it) =>
    num(it.onHand) <= 0
      ? "Out"
      : num(it.onHand) <= num(it.reorderPoint)
        ? "Low"
        : "OK";
  const stColor = {
    Out: "danger",
  ...
  const saveItem = () => {
    ...
    if (modal.type === "itemEdit")
      setProcInventory(inv.map((x) => (x.id === form.id ? rec : x)));
    else
      setProcInventory([
        ...inv,
        {
  ...
  const savePO = () => {
    ...
    if (modal.type === "poEdit")
      setProcPurchases(pos.map((x) => (x.id === form.id ? rec : x)));
    else
      setProcPurchases([
        ...pos,
        {
  ...
  const doDel = () => {
    if (modal.kind === "item")
      setProcInventory(inv.filter((x) => x.id !== modal.data.id));
    else setProcPurchases(pos.filter((x) => x.id !== modal.data.id));
    setModal(null);
  };
  const lowStock = inv.filter((it) => stockState(it) !== "OK");
  const invValue = inv.reduce(
    (a, it) => a + num(it.onHand) * num(it.unitValue),
    0,
  );
  ...
  const exportCSV = () => {
    ...
    if (tab === "purchases") {
      ...
      rows = pos
        .filter((p) => matchS([p.poNum, p.vendor, p.item]))
        .map((p) => [
          p.poNum,
          p.date,
          p.vendor,
      ...
      rows = inv
        .filter((it) => matchS([it.name, it.sku, it.category]))
        .map((it) => [
          it.name,
          it.sku,
          it.category,
          ...
          num(it.reorderPoint),
          num(it.unitValue),
          num(it.onHand) * num(it.unitValue),
          it.location,
          stockState(it),
        ]);
      name = "Inventory";
        ...
        React.createElement(
          ...
          it.issuedTo &&
            React.createElement(
              ...
              "\u2192 " +
                it.issuedTo +
                (it.receiverRole ? " (" + it.receiverRole + ")" : ""),
            ),
        ),
    },
    {
  ...
  if (tab === "inventory") {
    ...
    content = React.createElement(
      ...
      Table(
        rows,
        invCols,
        "No inventory items yet. Click \u201CAdd Item\u201D to stock the catalog.",
      ),
    );
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(SC, {
          ...
          label: "Pending",
          value: pos.filter(
            (p) => p.status === "Ordered" || p.status === "Shipped",
          ).length,
          color: c.cyan,
        }),
      ),
    ...
    const rows = lowStock
      .filter((it) => matchS([it.name, it.sku, it.category]))
      .sort((a, b) => num(a.onHand) - num(b.onHand));
    content = React.createElement(
      "div",
      null,
      React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "span",
          null,
          "Items at or below their reorder point \u2014 raise purchase orders to restock before they run out.",
        ),
      ),
      Table(
        rows,
        invCols,
        "All items are above their reorder point \u2014 nothing to restock. \u{1F389}",
      ),
    );
  } else if (tab === "suppliers") {
    const g = {};
    ...
    const issued = inv.filter(
      (it) =>
        (it.custodyStatus && it.custodyStatus !== "In Store") || it.issuedTo,
    );
    const rows = issued.filter((it) =>
      matchS([it.name, it.sku, it.issuedTo, it.receiverRole, it.issuedBy]),
    );
    content = React.createElement(
      "div",
      null,
      React.createElement(
        ...
        React.createElement(SC, {
          ...
          value: inv.filter(
            (it) =>
              it.custodyStatus === "Under Repair" ||
              it.custodyStatus === "Lost",
          ).length,
          color: c.danger,
        }),
        React.createElement(SC, {
  ...
  const itemModal =
    modal && (modal.type === "item" || modal.type === "itemEdit");
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Inventory, purchase orders, reorder alerts & suppliers",
        ),
      ),
      React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                modal.type === "itemEdit" ? "Edit Item" : "Add Inventory Item",
              ),
            ),
            React.createElement(
              "button",
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "itemEdit" ? "Save Item" : "Add Item",
            ),
          ),
        ),
      ),
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "poEdit" ? "Save PO" : "Create PO",
            ),
          ),
        ),
      ),
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "Delete this " +
              (modal.kind === "item" ? "item" : "purchase order") +
              "?",
          ),
          React.createElement(
            "p",
            ...
            React.createElement(
              ...
              modal.kind === "item" ? modal.data.name : modal.data.poNum,
            ),
