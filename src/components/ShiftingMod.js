const ShiftingMod = ({
  ...
  const filtered = useMemo(() => {
    ...
    if (search)
      d = d.filter((s) =>
        [s.student, s.lead, s.fromT, s.toT, s.reason].some((v) =>
          String(v).toLowerCase().includes(search.toLowerCase()),
        ),
      );
    if (fReason !== "all")
      d = d.filter((s) =>
        s.reason.toLowerCase().includes(fReason.toLowerCase()),
      );
    if (fSps !== "all")
      d = d.filter((s) => (fSps === "yes" ? s.sps === "yes" : s.sps !== "yes"));
    if (fDate === "week")
      d = d.filter(
        (s) => (today - new Date(s.date)) / (1000 * 60 * 60 * 24) <= 7,
      );
    if (fDate === "month")
      d = d.filter(
        (s) => (today - new Date(s.date)) / (1000 * 60 * 60 * 24) <= 30,
      );
    if (fDate === "year")
      d = d.filter(
        (s) => (today - new Date(s.date)) / (1000 * 60 * 60 * 24) <= 365,
      );
    return d.sort((a, b) => new Date(b.date) - new Date(a.date));
  }, [shifts, search, fReason, fSps, fDate]);
  const stats = useMemo(() => {
    const last30 = shifts.filter(
      (s) => (today - new Date(s.date)) / (1000 * 60 * 60 * 24) <= 30,
    );
    const last7 = shifts.filter(
      (s) => (today - new Date(s.date)) / (1000 * 60 * 60 * 24) <= 7,
    );
  ...
  const rc = useMemo(() => {
    const g = {};
    shifts.forEach((s) => {
      const k = s.reason.includes("Resign")
        ? "Resignation"
        : s.reason.includes("Terminated") || s.reason.includes("Fired")
          ? "Terminated"
          : s.reason.includes("Leave") || s.reason.includes("sick")
            ? "Leave/Sick"
            : s.reason.includes("Ramadan")
              ? "Ramadan"
              : s.reason.includes("Time")
                ? "Time Change"
                : s.reason.includes("Parent") || s.reason.includes("Request")
                  ? "Parent Req"
                  : "Other";
      g[k] = (g[k] || 0) + 1;
    });
  ...
  const tT = useMemo(() => {
    ...
    return {
      top_out: Object.entries(out)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8),
      top_in: Object.entries(inn)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 8),
    };
  }, [shifts]);
  ...
  const save = () => {
    ...
    if (modal.type === "new") {
      ...
      if (setTeachers && teachers && f.fromTime && f.toTime) {
        ...
        if (fromT && toT) {
          ...
          let existingCell = null;
          {
            ...
            if (cellOv && typeof cellOv === "object") existingCell = cellOv;
            else {
              for (const sh of ["Morning", "Evening", "Night", "Weekend"]) {
                const baseT = (TT_DATA[sh].teachers || []).find(
                  (x) => x.name === fromT.name || x.code === fromT.code,
                );
      ...
      const bookingChanged =
        ...
        (old.fromT !== f.fromT ||
          old.toT !== f.toT ||
          old.fromTime !== f.fromTime ||
          old.toTime !== f.toTime ||
          old.student !== f.student);
      if (
        bookingChanged &&
        old.fromT &&
        old.toT &&
        ...
        setTeachers(
          teachers.map((t) => {
            ...
            if (oldFromT && t.id === oldFromT.id && restoreCell) {
              const sch = result._ttSchedule
                ? JSON.parse(JSON.stringify(result._ttSchedule))
                : {};
  ...
  const del = () => {
    ...
    if (rec && rec.fromT && rec.toT && rec.fromTime && rec.toTime) {
      ...
      if (fromT && toT) {
        ...
        if (
          !confirm(
            "Delete this shift record AND reverse the booking move?\n\nThis will:\n\u2022 Free " +
              rec.toT +
              "\u0027s " +
              toPair.day +
              " " +
  ...
  const rColor = (r) =>
    r.includes("Resign") || r.includes("Terminated") || r.includes("Fired")
      ? "danger"
      : r.includes("Ramadan")
        ? "purple"
        : r.includes("Leave") || r.includes("sick")
          ? "warn"
          : r.includes("Parent") || r.includes("Request")
            ? "accent"
            : "cyan";
                ...
                React.createElement(
                  ...
                  React.createElement(
                    ...
                    React.createElement(
                      ...
                      React.createElement(ArrowRightLeft, {
                        size: 10,
                        color: c.accent,
                      }),
                      React.createElement(
                  ...
                  React.createElement(
                    ...
                    s.feedback
                      ? React.createElement(Badge, {
                          text:
                            s.feedback.length > 15
                              ? s.feedback.substring(0, 13) + ".."
                              : s.feedback,
                          color:
                            s.feedback.includes("Satisfied") &&
                            !s.feedback.includes("Not")
                              ? "success"
                              : s.feedback.includes("Not")
                                ? "danger"
                                : "warn",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              "Monthly Shift Volume (Last 12 months)",
            ),
            React.createElement(
              ResponsiveContainer,
              {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              "Top Losing Teachers (Students Transferred Away)",
            ),
            React.createElement(
              "div",
              {
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              "Top Receiving Teachers (Students Transferred To)",
            ),
            React.createElement(
              "div",
              {
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "SPS Pending (",
            shifts.filter((s) => s.sps !== "yes").length,
            ")",
          ),
          React.createElement(
            "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "Awaiting Feedback (",
            shifts.filter((s) => !s.feedback || s.feedback === "Pending")
              .length,
            ")",
          ),
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  "tr",
                  null,
                  ["Date", "Student", "From", "To", "Quick Action"].map((h) =>
                    React.createElement(
                      "th",
                      {
                        key: h,
              ...
              React.createElement(
                ...
                null,
                shifts
                  .filter((s) => !s.feedback || s.feedback === "Pending")
                  .slice(0, 15)
                  .map((s, i) =>
                    React.createElement(
                      "tr",
            ...
            React.createElement(
              ...
              React.createElement("div", {
                style: {
                  ...
                  borderRadius: 7,
                  background:
                    rColor(s.reason) === "danger"
                      ? c.danger
                      : rColor(s.reason) === "warn"
                        ? c.warn
                        : c.accent,
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "new" ? "New Class Shift" : "Edit Shift",
            ),
            React.createElement(
              "button",
              {
          ...
          React.createElement(
            ...
            "Record a class transfer from one teacher to another. Both teachers and time slots required.",
          ),
          React.createElement(
            "div",
            {
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              "Student Name * \u2014 " +
                (students || []).filter((s) => s.status !== "quit").length +
                " available",
            ),
            React.createElement("input", {
              ...
              onChange: (e) => {
                ...
                if (matched) {
                  setF({
                    ...
                    fromLead: (() => {
                      const tt = (teachers || []).find(
                        (t) => t.name === matched.teacher,
                      );
                      return tt
                        ? tt.teamLead || f.fromLead || ""
                        : f.fromLead || "";
                    })(),
                  });
                } else {
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                (teachers || [])
                  ...
                  .slice()
                  .sort((a, b) => computeFree(b).free - computeFree(a).free)
                  .map((t) => {
                    const cf = computeFree(t);
                    return React.createElement(
                      "option",
            ...
            (() => {
              ...
              for (const sh of ["Morning", "Evening", "Night", "Weekend"]) {
                const found = (TT_DATA[sh].teachers || []).find(
                  (x) => x.name === toTeacher.name || x.code === toTeacher.code,
                );
                if (found) {
                  tShift = sh;
              ...
              if (allFreeOpts.length === 0) {
                return React.createElement(
                  ...
                  React.createElement(
                    ...
                    "\u26A0 " +
                      f.toT +
                      " has no free slots. Pick a different New Teacher.",
                  ),
                );
              }
              return React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "new" ? "Create Shift" : "Save Changes",
            ),
          ),
        ),
      ),
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.data.reason || "Not specified",
            ),
          ),
          React.createElement(
            "div",
            ...
            React.createElement(
              ...
              modal.data.feedback
                ? React.createElement(Badge, {
                    ...
                    color:
                      modal.data.feedback.includes("Satisfied") &&
                      !modal.data.feedback.includes("Not")
                        ? "success"
                        : modal.data.feedback.includes("Not")
                          ? "danger"
                          : "warn",
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            "? This is a historical record and cannot be recovered.",
          ),
          React.createElement(
            "div",
            {
        ...
        const doBulkShift = () => {
          ...
          const confirmMsg =
            ...
            (conflicts.length > 0
              ? "\n\n\u26A0 " +
                conflicts.length +
                " will be SKIPPED due to slot conflicts."
              : "") +
            "\n\nThis will:\n\u2022 Free " +
            transfers.length +
            " slot" +
          ...
          if (setStudents) {
            const transferredNames = new Set(
              transfers.map((b) => (b.cell.s || "").trim().toLowerCase()),
            );
            setStudents(
              students.map((s) => {
          ...
          setTimeout(() => {
            ...
            if (conflicts.length > 0) {
              ...
              msg +=
                "\n\nThese students remain with " +
                f.bulkFromT +
                ". Use New Shift to move them individually.";
            }
            alert(msg);
          }, 100);
        };
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                "Bulk Shift \u2014 Transfer Entire Class Load",
              ),
              React.createElement(
                "button",
                {
            ...
            React.createElement(
              ...
              "Use when a teacher resigns or leaves. ALL their booked classes will transfer to the new teacher in one operation. Slot conflicts will be skipped and reported.",
            ),
            React.createElement(
              "div",
              {
            ...
            f.bulkFromT &&
              React.createElement(
                ...
                React.createElement(
                  ...
                  "Classes to transfer (" + fromBookings.length + ")",
                  ...
                  React.createElement(
                    ...
                    React.createElement(
                      ...
                      React.createElement(
                        ...
                        ["DAY", "PKT SLOT", "STUDENT", "COURSE", "STATUS"].map(
                          (h) =>
                            React.createElement(
                              "th",
                              {
                    ...
                    React.createElement(
                      "tbody",
                      null,
                      fromBookings.map((b, i) => {
                        const toTe = (teachers || []).find(
                          (x) => x.name === f.bulkToT,
                        );
                        const conflict = toTe
                          ? checkConflict(toTe, b.day, b.slot)
                          : false;
                        return React.createElement(
                          "tr",
                          {
            ...
            React.createElement(
              ...
              React.createElement(
                Btn,
                {
                  ...
                  icon: ArrowRightLeft,
                  disabled:
                    !f.bulkFromT || !f.bulkToT || fromBookings.length === 0,
                },
                "Transfer " +
                  (fromBookings.length || "0") +
                  " Class" +
                  (fromBookings.length === 1 ? "" : "es"),
              ),
            ),
          ),
        );
