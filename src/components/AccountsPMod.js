const AccountsPMod = ({ apLiabilities, setApLiabilities }) => {
  ...
  const isOverdue = (l) =>
    l.status !== "paid" && l.dueDate && String(l.dueDate) < today;
  const paidOf = (l) =>
    l.status === "paid"
      ? num(l.amount)
      : l.status === "partial"
  ...
  const save = () => {
    ...
    if (modal.type === "edit")
      setApLiabilities(liabs.map((l) => (l.id === form.id ? rec : l)));
    else
      setApLiabilities([
        ...liabs,
        {
  ...
  const exportCSV = () => {
    ...
    if (tab === "vendors") {
      ...
      rows = liabs
        .filter((l) => matchS([l.vendor, l.category, l.reference]))
        .map((l) => [
          l.vendor,
          l.category,
          num(l.amount),
  ...
  if (tab === "ledger") {
    ...
    if (fStatus !== "all")
      rows =
        fStatus === "overdue"
          ...
          : rows.filter((l) => l.status === fStatus);
    ...
    const rows = outstanding
      .filter((l) => matchS([l.vendor, l.category]))
      .sort((a, b) =>
        String(a.dueDate || "9999") < String(b.dueDate || "9999") ? -1 : 1,
      );
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "span",
          null,
          "Upcoming payment obligations grouped by the month they're due \u2014 plan cash-flow ahead.",
        ),
      ),
      Table(
        rows,
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Liabilities ledger, vendor dues, aging & payment schedule",
        ),
      ),
      React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                modal.type === "edit" ? "Edit Liability" : "Add Liability",
              ),
            ),
            React.createElement(
              "button",
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              modal.type === "edit" ? "Save Changes" : "Add Liability",
            ),
          ),
        ),
      ),
