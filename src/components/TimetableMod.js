const TimetableMod = ({
  ...
  const ttStage = (cell) => {
    ...
    const _st = (students || []).find(
      (x) => x.name === cell.s && (x.trial || x.fee === "trial"),
    );
    if (!_st) return null;
    const _pr = _st.attended || 0;
    const _paid =
      _st.fee === "paid" ||
      (arPayments || []).some((p) => String(p.studentId) === String(_st.id));
    ...
    return {
      bg: "color-mix(in srgb, var(--danger) 35%, transparent)",
      bd: c.danger,
    };
  };
  ...
  const saveBooking = () => {
    ...
    teachers.forEach((otherT) => {
      ...
      const otherCell =
        otherT.schedule && otherT.schedule[bD] && otherT.schedule[bD][bS];
      if (
        otherCell &&
        typeof otherCell === "object" &&
        otherCell.s &&
        otherCell.s.toLowerCase() === bookForm.s.toLowerCase()
      ) {
    ...
    if (conflicts.length > 0) {
      if (
        !confirm(
          '\u26A0 CONFLICT DETECTED!\n\nStudent "' +
            bookForm.s +
            '" is already booked at this same time (' +
            bD +
            " " +
            bS +
            ") with:\n\n• " +
            conflicts.join("\n• ") +
            "\n\nThis would create a double-booking. Continue anyway?",
        )
      )
        return;
    }
  ...
  const removeBooking = () => {
    ...
    if (
      oldCell &&
      typeof oldCell === "object" &&
      oldCell.s &&
      setStudents &&
      students
    ) {
      const sNameLc = String(oldCell.s).trim().toLowerCase();
      const sParentLc = String(oldCell.l || "")
        .trim()
        .toLowerCase();
      setStudents(
        students.map((s) => {
          const nm = String(s.name || "")
            .trim()
            .toLowerCase();
          const pa = String(s.parent || "")
            .trim()
            .toLowerCase();
          if (
            nm === sNameLc &&
            (!sParentLc || pa === sParentLc) &&
            s.teacher === bT.name
          ) {
            return {
              ...s,
  ...
  const pasteCell = (targetTeacher, targetDay, targetSlot, existingCell) => {
    if (!clipboard) return;
    const doIt = () => {
      if (!targetTeacher.schedule[targetDay])
        targetTeacher.schedule[targetDay] = {};
      targetTeacher.schedule[targetDay][targetSlot] = clipboard._free
        ? "F"
        : {
            ...clipboard,
          };
  ...
  const copyMonToWeekdays = (targetTeacher) => {
    ...
    if (
      !confirm(
        "Copy Monday\u2019s entire schedule to Tue, Wed, Thu, and Fri for " +
          targetTeacher.name +
          "?\n\nExisting bookings on those days will be replaced.",
      )
    )
      return;
  ...
  const printSchedule = (targetTeacher) => {
    ...
    let html =
      "<html><head><title>" +
      tName +
      " Schedule</title><style>body{font-family:sans-serif;padding:20px}h1{margin-bottom:6px}.sub{color:#666;margin-bottom:18px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #aaa;padding:6px 8px;font-size:11px;text-align:left;vertical-align:top}th{background:#eee}.free{color:#0a0}.booked{background:#f0f4ff}@media print{body{padding:0}}</style></head><body>";
    html +=
      "<h1>" +
      tName +
      "</h1><div class=sub>Code: " +
      escHTML(targetTeacher.code) +
      " \u00B7 " +
      escHTML(targetTeacher.location || "") +
      " \u00B7 " +
      escHTML(shift) +
      " Shift \u00B7 Lead: " +
      escHTML(targetTeacher.lead || "-") +
      "</div>";
    ...
    days.forEach((d) => {
      ...
      slts.forEach((s) => {
        const cell =
          targetTeacher.schedule &&
          targetTeacher.schedule[d] &&
          targetTeacher.schedule[d][s];
        if (cell && typeof cell === "object" && cell.s) {
          html +=
            "<td class=booked><b>" +
            escHTML(cell.s) +
            "</b>" +
            (cell.a ? " (" + escHTML(cell.a) + ")" : "") +
            "<br><small>" +
            escHTML(cell.c || "") +
            "</small><br><small>" +
            escHTML(cell.t || "") +
    ...
    html +=
      '</tbody></table><div style="margin-top:14px;font-size:10px;color:#999">Generated ' +
  ...
  const days =
    shift === "Weekend" ? ["Sat", "Sun"] : ["Mon", "Tue", "Wed", "Thu", "Fri"];
  ...
  const _mN = String((user && user.name) || "")
    .trim()
    .toLowerCase();
  const _mC = String(
    (appTeachers && appTeachers[0] && appTeachers[0].code) || "",
  ).trim();
  const teachers =
    user && user.role === "teacher" && _mN
      ? [...mergedBase, ...extras].filter(
          (t) =>
            String(t.name || "")
              .trim()
              .toLowerCase() === _mN ||
            (t.code && _mC && String(t.code).trim() === _mC),
        )
      : [...mergedBase, ...extras];
  ...
  const currentTeacher = teachers[teacherIdx] ||
    teachers[0] || {
  ...
  const busiest = [...shiftStats.perTeacher]
    .sort((a, b) => b.booked - a.booked)
    .slice(0, 5);
  const mostAvailable = [...shiftStats.perTeacher]
    .filter((t) => t.free > 0)
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(SC, {
        ...
        value:
          Math.round(
            (shiftStats.booked / (shiftStats.booked + shiftStats.free || 1)) *
              100,
          ) + "%",
        color:
          shiftStats.booked / (shiftStats.booked + shiftStats.free || 1) >= 0.85
            ? c.danger
            : c.purple,
      }),
      React.createElement(SC, {
    ...
    React.createElement(
      ...
      [
        ...
        ["overview", "All Teachers Overview"],
        ["smartfinder", "Smart Finder"],
        ["analytics", "Analytics"],
      ]
        .filter(
          (tb) => !(user && user.role === "teacher") || tb[0] === "individual",
        )
        .map(([k, l]) =>
          React.createElement(
            "button",
      ...
      React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                React.createElement(
                  ...
                  "Click any cell to paste here. Drag any cell (booked or free) to copy.",
                ),
              ),
            ),
            React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(SearchableSelect, {
            value: teacherIdx,
            onChange: (val) => setTeacherIdx(parseInt(val)),
            options: filteredTeachers.map((t) => {
              const realIdx = teachers.findIndex(
                (x) => x.code === t.code && x.name === t.name,
              );
              return {
                value: realIdx,
                label:
          ...
          React.createElement(
            ...
            {
              onClick: () =>
                setTeacherIdx(Math.min(teachers.length - 1, teacherIdx + 1)),
              ...
              style: {
                ...
                borderRadius: 6,
                cursor:
                  teacherIdx >= teachers.length - 1 ? "not-allowed" : "pointer",
                padding: "7px 10px",
                color:
                  teacherIdx >= teachers.length - 1 ? c.textMuted : c.accent,
              ...
              React.createElement(
                "button",
                {
                  ...
                  style: {
                    ...
                    border:
                      "1px solid " + (cellFilter === f ? c.accent : c.border),
                    borderRadius: 5,
                    cursor: "pointer",
        ...
        React.createElement(
          ...
          [
            ["Trial \u00B7 new (<3 classes)", c.danger],
            ["Trial passed \u00B7 awaiting payment", c.warn],
            ["Active \u00B7 paid", c.accent],
          ].map((p, pi) =>
            React.createElement(
              "span",
              {
                      ...
                      React.createElement(
                        "div",
                        {
                          ...
                          style: {
                            ...
                            minHeight: 78,
                            cursor:
                              (cell && cell.s) || cell === "F" || !cell
                                ? "pointer"
                                : "default",
                            transition: "all .15s",
                            opacity:
                              (cellFilter === "booked" && !(cell && cell.s)) ||
                              (cellFilter === "free" && cell && cell.s)
                                ? 0.25
                                : 1,
                            boxShadow:
                              studentSearch &&
                        ...
                        !cell &&
                          ...
                          React.createElement(
                            ...
                            React.createElement("div", null, "Same as Mon"),
                            React.createElement(
                              "div",
                              {
                                style: {
                            ...
                            cell.s !== "Self" && cell.l
                              ? React.createElement(
                                  ...
                                  (() => {
                                    ...
                                    const k = Object.keys(sc).find((x) =>
                                      String(_cs).includes(x),
                                    );
                                    return k
                                      ? React.createElement("span", {
                                          style: {
                              ...
                              React.createElement(
                                ...
                                cell.f.map((flag, fi) => {
                                  const fc =
                                    ...
                                    flag.includes("ADHD")
                                      ? [c.dangerBg, c.danger]
                                      : flag.includes("Female")
                                        ? [c.purpleBg, c.purple]
                                        : flag.includes("Ramadan") ||
                                            flag.includes("Leave")
                                          ? [c.warnBg, c.warn]
                                          : flag.includes("Camera") ||
                                              flag.includes("15-min")
                                            ? [c.cyanBg, c.cyan]
                                            : [c.warnBg, c.warn];
                                  return React.createElement(
                                    "span",
        ...
        React.createElement(
          ...
          [
            ["Booked", c.accentBg, c.accent],
            ["Free", c.successBg, c.success],
            ["Special Note", c.warnBg, c.warn],
            ["No Class", c.bgDeep, c.textMuted],
            ["Same as Mon", "transparent", c.textMuted],
          ].map(([l, bg, col]) =>
            React.createElement(
              "div",
              {
          ...
          React.createElement(
            ...
            "Tip: Tue-Fri slots without changes inherit Monday schedule",
          ),
        ),
      ),
    mode === "overview" &&
            ...
            shiftStats.perTeacher.map((t, i) =>
              React.createElement(
                ...
                React.createElement(
                  ...
                  React.createElement("span", null, "Booked: ", t.booked),
                  React.createElement("span", null, "Free: ", t.free),
                ),
              ),
            ),
          ),
    ...
    mode === "smartfinder" && React.createElement(SmartFinder, null),
    quickBook &&
      React.createElement(
        "div",
        {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              quickBook.step === 1
                ? "Book Slot — Select Slot"
                : "Book Slot — Student Details",
            ),
            React.createElement(
              "button",
              {
          ...
          quickBook.step === 1 &&
            React.createElement(
              ...
              React.createElement(
                ...
                "Select a teacher, day, and free time slot to book a new class in the ",
                shift,
                " shift.",
              ),
              React.createElement(
                ...
                React.createElement(SearchableSelect, {
                  value: qbTeacher,
                  onChange: (val) => setQbTeacher(parseInt(val)),
                  options: teachers.map((t, i) => ({
                    value: i,
                    label:
                      "#" +
                      t.sno +
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              bookModal.editing ? "Edit Booking" : "Book Class Slot",
            ),
            React.createElement(
              "button",
              {
          ...
          React.createElement(
            "div",
            {
              style: {
                ...
                border:
                  "1px solid " +
                  (bookModal.editing ? c.purple : c.success) +
                  "44",
              },
            },
            React.createElement(
              ...
              bookModal.editing
                ? "BOOKED CLASS · EDITING"
                : "FREE SLOT · NEW BOOKING",
            ),
            React.createElement(
              "div",
              {
              ...
              "Teacher: ",
              bookModal.teacher.name,
              " (",
              bookModal.teacher.code,
              ")",
            ),
            React.createElement(
              "div",
          ...
          React.createElement(
            ...
            bookForm.country === "Other"
              ...
              : React.createElement(Inp, {
                  ...
                  options: [
                    "",
                    ...(COUNTRY_STATES[bookForm.country || "USA"] || []),
                  ],
                }),
          ),
          React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                bookModal.editing ? "Update Booking" : "Book Class",
              ),
            ),
          ),
        ),
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                (() => {
                  const _state =
                    cellModal.cell.state ||
                    (cellModal.cell.s === "Self"
                      ? cellModal.cell.l
                      : (
                          initStudents.find(
                            (x) =>
                              ...
                              cellModal.cell.s &&
                              x.name.toLowerCase() ===
                                String(cellModal.cell.s).toLowerCase(),
                          ) || {}
                        ).state || cellModal.cell.l);
          ...
          React.createElement(
            ...
            [
              cellModal.cell.s !== "Self"
                ? ["Parent / Lead", cellModal.cell.l]
                : null,
              [
                "Student",
                cellModal.cell.s +
                  (cellModal.cell.a ? " (" + cellModal.cell.a + "y)" : ""),
              ],
              ["Course", cellModal.cell.c],
              cellModal.cell.family
                ? ["Family Group", cellModal.cell.family]
                : null,
              cellModal.cell.s === "Self"
                ? ["Parent / Lead", cellModal.cell.l]
                : null,
              ["USA Time", cellModal.cell.t || "Not set"],
              [
                "Pakistan Time",
                (() => {
                  const _state =
              ...
              ["Shift", shift],
              ["Code", cellModal.teacher.code],
              cellModal.cell.country
                ? ["Country", cellModal.cell.country]
                : null,
              cellModal.cell.state
                ? ["State / Province", cellModal.cell.state]
                : null,
              cellModal.cell.gender && cellModal.cell.gender !== "Any"
                ? ["Gender Pref", cellModal.cell.gender]
                : null,
              cellModal.cell.phone ? ["Phone", cellModal.cell.phone] : null,
            ]
              .filter(Boolean)
              .map(([l, v]) =>
                React.createElement(
          ...
          cellModal.cell.f &&
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                cellModal.cell.f.map((fl, i) =>
                  React.createElement(Badge, {
                    key: i,
                    text: fl,
                    color:
                      fl.includes("Don't") || fl.includes("ADHD")
                        ? "danger"
                        : fl.includes("Female")
                          ? "purple"
                          : fl.includes("Ramadan") || fl.includes("Leave")
                            ? "warn"
                            : fl.includes("Camera") || fl.includes("15-min")
                              ? "cyan"
                              : "accent",
                  }),
                ),
                  ...
                  onClick: () => {
                    const stu = (students || []).find(
                      (s) =>
                        ...
                        (cellModal.cell.s || "").toLowerCase().trim(),
                    );
                    setProgressForm({
                      attended: "present",
