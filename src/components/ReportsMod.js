const ReportsMod = ({
  ...
  const monthRevenue = PAY.filter((p) => inMonth(p.paidDate)).reduce(
    (a, p) => a + num(p.amount),
    0,
  );
  const monthlySalaries = activeT.reduce((a, t) => a + num(t.salary), 0);
  const monthAPpaid = LIA.filter(
    (l) => l.status !== "unpaid" && inMonth(l.paidDate),
  ).reduce(
    (a, l) => a + (l.status === "paid" ? num(l.amount) : num(l.paidAmount)),
    0,
  );
  const monthExpenses = monthlySalaries + monthAPpaid;
  const monthNet = monthRevenue - monthExpenses;
  const apOutstanding = LIA.reduce(
    (a, l) =>
      a +
      (l.status === "paid"
        ? 0
        : Math.max(
            0,
            num(l.amount) - (l.status === "partial" ? num(l.paidAmount) : 0),
          )),
    0,
  );
  const newAdmitsMonth = S.filter((s) => inMonth(s.dor)).length;
  const qcOpen = QC.filter(
  ...
  const certs = TR.flatMap((p) => p.attendees || []).filter(
  ...
  const monthsSeries = () => {
    ...
    return Object.keys(set)
      .sort((a, b) => (a < b ? 1 : -1))
      .map((m) => {
        const rev = PAY.filter(
          (p) => String(p.paidDate || "").slice(0, 7) === m,
        ).reduce((a, p) => a + num(p.amount), 0);
        const ap = LIA.filter(
          (l) =>
            l.status !== "unpaid" && String(l.paidDate || "").slice(0, 7) === m,
        ).reduce(
          (a, l) =>
            a + (l.status === "paid" ? num(l.amount) : num(l.paidAmount)),
          0,
        );
  ...
  if (tab === "overview") {
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        card(Users, "Active Students", activeS.length, c.success),
        card(
          GraduationCap,
          "New Admits (" + mLabel(month) + ")",
          newAdmitsMonth,
          c.accent,
        ),
        card(Coffee, "On Leave", leaveS.length, c.warn),
        card(UserX, "Quit (total)", quitS.length, c.danger),
      ),
      React.createElement(
        "h3",
        {
      ...
      React.createElement(
        ...
        card(Receipt, "Expenses", "Rs " + fmt(monthExpenses), c.warn),
        card(
          TrendingUp,
          "Net Profit",
          "Rs " + fmt(monthNet),
          monthNet >= 0 ? c.success : c.danger,
        ),
        card(
          AlertTriangle,
          "Payables Outstanding",
          "Rs " + fmt(apOutstanding),
          c.danger,
        ),
      ),
      React.createElement(
      ...
      React.createElement(
        ...
        card(Users, "Active Teachers", activeT.length, c.accent),
        card(Target, "Sales Leads (" + mLabel(month) + ")", leadsMonth, c.cyan),
        card(Award, "Conversions", conversions, c.success),
        card(
          Shield,
          "Open QC Cases",
          qcOpen.length,
          qcOpen.length ? c.danger : c.textMuted,
        ),
      ),
    );
    ...
    const funnel = [
      ["New", REF.filter((r) => r.status === "New").length],
      ["Contacted", REF.filter((r) => r.status === "Contacted").length],
      [
        "Trial Scheduled",
        REF.filter((r) => r.status === "Trial Scheduled").length,
      ],
      ["Enrolled", REF.filter((r) => r.status === "Enrolled").length],
      ["Declined", REF.filter((r) => r.status === "Declined").length],
    ];
    const convRate = REF.length
      ? Math.round((conversions / REF.length) * 100)
      : 0;
    content = React.createElement(
      "div",
      null,
            ...
            render: (x) =>
              React.createElement(
                ...
                React.createElement("div", {
                  style: {
                    width:
                      (REF.length ? Math.round((x.n / REF.length) * 100) : 0) +
                      "%",
                    height: "100%",
                    background: c.accent,
                  },
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(SC, {
          ...
          value:
            (monthRevenue > 0
              ? Math.round((monthNet / monthRevenue) * 100)
              : 0) + "%",
          color: monthNet >= 0 ? c.success : c.danger,
        }),
      ),
      React.createElement(
        ...
        React.createElement(
          "span",
          null,
          "Revenue from fee collections, expenses = staff salaries (Rs ",
          fmt(monthlySalaries),
          "/mo) + paid liabilities. Auto-built from live data.",
        ),
      ),
      Table(
        series,
              ...
              React.createElement(
                ...
                React.createElement("div", {
                  style: {
                    width:
                      (monthRevenue
                        ? Math.round((x.sum / monthRevenue) * 100)
                        : 0) + "%",
                    height: "100%",
    ...
    content = React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(SC, {
          ...
          label: "Quit Rate",
          value: S.length
            ? Math.round((quitS.length / S.length) * 100) + "%"
            : "0%",
          color: c.warn,
        }),
      ),
    ...
    const rows = LV.slice().sort((a, b) =>
      String(lf(a, ["from", "lvFrom", "startDate", "date"])) <
      String(lf(b, ["from", "lvFrom", "startDate", "date"]))
        ? 1
        : -1,
    );
    const pending = LV.filter(
      (l) => String(lf(l, ["status"])).toLowerCase() === "pending",
    ).length;
    content = React.createElement(
      "div",
      null,
      ...
      Table(
        ...
        [
          ...
          },
          {
            h: "From",
            render: (l) =>
              lf(l, ["from", "lvFrom", "startDate", "date"]) || "\u2014",
          },
          {
            h: "To",
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          "Executive command center \u2014 live KPIs across every module",
        ),
      ),
